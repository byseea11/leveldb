...
# leveldb-benchmark
leveldb的benchmark实际上就是调用了google benchmark和google test的三方库，将提供的函数接口进行封装做成的功能。

涉及的代码文件：
1. benchmarks/*: leveldb-benchmark实现代码部分
2. doc/benchmark.html: leveldb提供的官方测试结果

接下来仔细分析leveldb提供了一些什么功能
1. write
   1. fillsql: 异步顺序填充，key是从1,2,3,...,n
   
   ```cpp
   // 这个函数的实现会生成一系列的数据，然后调用Put对其进行插入
